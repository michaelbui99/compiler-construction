...
describe("Checker tests", () => {
    ...
    test("access variable in an outer scope is illegal", () => {
        ...
        expect(checkerHelper(source)).toThrow(new CompilerError("Cannot reassign a as it has never been declared"));
    });

    test("assign value has to be same type", () => {
        const source = "let a 25 % ass a \"Hi\" %";
        expect(checkerHelper(source)).toThrow(new CompilerError("Assignment of type STRING not possible to type INTEGER"));
    });
    ...
    test("integers are not indexable", () =>{
        ...
        expect(checkerHelper(source)).toThrow(new CompilerError("Depth of variable does not correspont to indexes"));
    });
    ...
    test("no operators possible for strings", ()=>{
        ...
        expect(checkerHelper(source)).toThrow(new CompilerError("operatior and can not be applies to hallo and  world"));
    });

    test("equals not possible on strings", ()=>{
        const source = "iff \"hi\" eql \"Hi\" thn end";
        expect(checkerHelper(source)).toThrow(new CompilerError("operatior eql can not be applies to hi and Hi"));
    });
    ...
    test("all arrays are just integers", () =>{
        ...
        expect(checkerHelper(source)).toThrow(new CompilerError("Arrays can have elements of type array or integer only."));
    });

    test("len can be used on anything except of strings", ()=>{
        const source = "let a 8 % let b run len a end % let myArr arr 1 2 3 4 % ass b len myArr% ass b run len tru end %"
        checkerHelper(source);
    });
